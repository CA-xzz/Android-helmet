...
- 人员和位置字段已进入设备数据库和后台归档协议；当前没有人员绑定和定位输入，按规则保存为 `null` 与 `NO_FIX`，不能作为元数据最终一致性证据。
- 应用私有配置中的 Bearer token 只用于开发联调，外部环境应使用受控设备凭据。
- 开发板系统时间仍不正确，板端日志日期为 2026-07-29。证据目录的验证日期使用主机时间。

## 最终验收缺口

阶段 3 的软件、开发板无摄像头故障处理和本地端到端上传门禁已通过。OQ-001、OQ-002、OQ-010、OQ-012 和 OQ-013 仍阻塞真实物理键、13MP 照片、连续 1080P、音频、长稳、温升、真实断网补传、人员和位置绑定及外部对象存储。因此 REQ-5.2-01 至 REQ-5.2-06 仍不得标记为最终完成。
