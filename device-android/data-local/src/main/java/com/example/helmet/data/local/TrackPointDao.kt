...
interface TrackPointDao {
    ...
    @Query("SELECT COALESCE(MAX(sequence), 0) + 1 FROM track_points WHERE deviceId = :deviceId")
    ...
    @Query("UPDATE track_points SET deliveryState = 'DELIVERED', deliveredAtEpochMillis = :deliveredAt, lastError = NULL WHERE messageId = :messageId")
