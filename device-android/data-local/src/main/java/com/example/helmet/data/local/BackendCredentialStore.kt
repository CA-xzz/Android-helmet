...
internal class BackendCredentialStore(context: Context) {
    ...
    fun write(value: String) {
        if (value.isBlank()) {
            check(preferences.edit().clear().commit()) { "failed to clear backend credential" }
            return
        }
        val cipher = Cipher.getInstance(TRANSFORMATION)
    ...
    companion object {
        internal const val PREFERENCES_NAME = "helmet_backend_credentials"
        internal const val LEGACY_PREFERENCES_NAME = "helmet_runtime_config"
        internal const val LEGACY_TOKEN_KEY = "backend_bearer_token"
        private const val KEYSTORE_PROVIDER = "AndroidKeyStore"
        private const val KEY_ALIAS = "helmet_backend_bearer_aes_v1"
